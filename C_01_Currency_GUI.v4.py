...
class Converter:
    ...
    def __init__(self, parent):
        ...
        # GUI Frame
        self.currency_frame = Frame(parent, padx=10, pady=10)
        ...
        # Conversion buttons (row 4)
        self.button_frame = Frame(self.currency_frame)
        self.button_frame.grid(row=4)

        # Button list: [text, bg, command, row, column]
        button_details_list = [
        ...
        for item in button_details_list:
            Button(self.button_frame,
                   ...
                   width=12, command=item[2]).grid(row=item[3], column=item[4], padx=5, pady=5)

    def check_currency(self, convert_mode):
        """
        Validates input and converts currency
        """
    ...
    def convert(self, convert_mode, to_convert):
        """
        Converts currency based on selected mode
        """
        if convert_mode == 0:
